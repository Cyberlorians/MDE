...
$healthData = @()
try {
    $query = "DeviceTvmInfoGathering | where Timestamp > ago(7d) | extend AF = parse_json(AdditionalFields) | extend AvScan = parse_json(tostring(AF.AvScanResults)) | summarize arg_max(Timestamp, *) by DeviceId | project computerDnsName = DeviceName, lastSeenTime = LastSeenTime, avMode = tostring(AF.AvMode), avEngineVersion = tostring(AF.AvEngineVersion), avEngineUpdateTime = tostring(AF.AvEngineUpdateTime), avIsEngineUpToDate = tostring(AF.AvIsEngineUptodate), avSignatureVersion = tostring(AF.AvSignatureVersion), avSignatureUpdateTime = tostring(AF.AvSignatureUpdateTime), avIsSignatureUpToDate = tostring(AF.AvIsSignatureUptoDate), avPlatformVersion = tostring(AF.AvPlatformVersion), avPlatformUpdateTime = tostring(AF.AvPlatformUpdateTime), avIsPlatformUpToDate = tostring(AF.AvIsPlatformUptodate), quickScanTime = tostring(AvScan.Quick.Timestamp), quickScanResult = tostring(AvScan.Quick.ScanStatus), quickScanError = tostring(AvScan.Quick.ErrorCode), fullScanTime = tostring(AvScan.Full.Timestamp), fullScanResult = tostring(AvScan.Full.ScanStatus), fullScanError = tostring(AvScan.Full.ErrorCode)"
